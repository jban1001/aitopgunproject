...
|---|---|---|---|
| A | 후방 우세에서 조준 원뿔과 사격창 생성 | WEZ dwell, 실제 damage, 생존 | 불리한 위치 회복 |
...
| D | 상대가 뒤를 잡은 상태에서 위협 해제 | 피격 억제, 생존, 위협각 이탈 | 장시간 공격 추격 |
| E | 후방 위치 우세 형성 | 상대 후방 진입, WEZ로의 인계, 에너지 유지 | 최종 정밀 조준 |
| F | 정면 merge 사격 부정 후 B/C 영역으로 인계 | incoming 방어, 생존, 180 m/s 이상, transfer dwell | post-cross 공격 완성 |

## A/D/E 계보

- A1-A4는 사격 원뿔을 단계적으로 좁혔고 A5는 실제 gun 시나리오를 추가했습니다.
- D1-D4는 깊은 후방 위협에서 시작해 더 가까운 위협과 다양한 에너지 상태로 확장했습니다.
- E1-E7은 단순 후방 추적에서 Viewer에서 관측된 위치/에너지 변형까지 확장했습니다.
- 초기 A/D/E shell 사다리는 `scripts/legacy/`에 역사 자료로 보존했습니다. 로컬 절대 경로와 오래된 종료 방식이 있으므로 현재 orchestrator의 예제로 직접 사용하지 않습니다.
