...
$OrchestratorLog = Join-Path $LogRoot "F_series_orchestrator.log"
...
function Write-Status([string]$Message) {
    $line = "[{0}] [F] {1}" -f (Get-Date -Format "MM-dd HH:mm:ss"), $Message
...
function Get-Score([string]$Tag, [int]$RunMinimum) {
    ...
    $json = & $Python (Join-Path $Root "scripts\f_curriculum_score.py") $csv --threshold $ScoreThreshold --min-iteration $RunMinimum
...
function Invoke-FTrainingRun(
    ...
    while (-not $process.HasExited) {
        ...
        if ($score.current_iter -ne $null -and [int]$score.current_iter -ne $lastIteration) {
            ...
            if ($score.ready) {
                Write-Status ("{0} iter={1} score={2:N3} cross={3:N3} handoff={4:N3} threat={5:N3} incoming={6:N3} offense={7:N3} plateau={8}" -f $Tag, $score.current_iter, $score.current.score, $score.current.cross, $score.current.handoff, $score.current.threat_clear, $score.current.incoming_clear, $score.current.offense, $score.plateau_age)
    ...
    if ($final.ready) {
        ...
        $final | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $LogRoot "$Tag\F_stage_summary.json") -Encoding utf8
    }
    return [pscustomobject]@{ Tag=$Tag; Final=$final; Qualified=$qualified; RequestedStop=$requestedStop }
}

New-Item -ItemType Directory -Force -Path $LogRoot | Out-Null
Write-Status "F-series start: stages=$($Stages -join ',') min=$MinIterations continuation-min=$ContinuationMinIterations plateau=$PlateauPatience threshold=$ScoreThreshold parent=$InitialBundle"
...
foreach ($stage in $Stages) {
    ...
    $run = Invoke-FTrainingRun $tag $experiment $parent $false $MinIterations 300
    if (-not $run.Qualified) {
        ...
        $run = Invoke-FTrainingRun $btag $experiment $checkpoint $true $ContinuationMinIterations 220
    }
    if (-not $run.Qualified) {
...
Write-Status "F-series orchestrator finished; final parent=$parent"
