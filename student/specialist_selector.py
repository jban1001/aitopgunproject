...
class SpecialistSelector(ActionProvider):
    ...
    def _decide(self, own_ata: float, tgt_ata: float, dist: float, alt_m: float) -> str:
        ...
        # 2) 정면 merge: 두 기수가 서로를 향하는 F1의 검증 범위.
        if (
            ...
            return self.FRONT
        # 3) 교차 방어: C5b 최종 시나리오가 검증한 좁은 위협 띠만 맡긴다.
        if (
        ...
        if (
            ...
            return self.POS
        # 6) 교량: 사격/위치/방어 어느 쪽도 아닌 중립 교착을 다음 전문가 띠로 운반한다.
        if (
    ...
    def compute_action(self, context: ActionContext) -> ActionResult:
        ...
        # **비대칭 히스테리시스.**
        #  - BT 로 돌아가는 것은 **즉시**. 조건이 깨졌는데 붙들고 있으면 고도를 태운다.
        ...
        if below_floor:
            ...
            # 조건이 깨지거나 사격창에서 벗어나면 어떤 유지 카운터보다 먼저 BT로 복귀한다.
            new_mode = self.BT
            self._held = 0
        elif want == self.GUN:
